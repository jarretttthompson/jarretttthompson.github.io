/**
 * Dev-only toolbar — NEVER merge to main.
 * Listed in /.dev-only and guarded by git hooks.
 */
(function initDevToolbar() {
  ...
  const pages = [
    ...
    { label: 'Resume', href: 'page2.html' },
    { label: 'Artwork', href: 'page3.html' },
    { label: 'Projects', href: 'page4.html' },
    { label: 'Compositions', href: 'page5.html' },
    { label: 'Music', href: 'music.html' },
    { label: 'Photos', href: 'photo-album.html' },
    { label: 'Friends', href: 'friends.html' },
    { label: 'Playlists', href: 'playlists.html' },
  ...
  /* ── Inject styles via a <style> in <html>, outside body's filter ── */
  ...
  style.textContent = `
    ...
    #dev-toolbar .dt-sep {
      width: 1px;
      height: 18px;
      background: rgba(224, 111, 234, 0.3);
      flex-shrink: 0;
    }
    #dev-toolbar .dt-pages {
      display: flex;
      gap: 2px;
      flex-wrap: nowrap;
      align-items: center;
    ...
    #dev-toolbar .dt-pages a:hover {
      background: rgba(224, 111, 234, 0.2);
      color: #fff;
    }
    #dev-toolbar .dt-pages a.dt-active {
      background: rgba(224, 111, 234, 0.25);
      color: #e06fea;
      font-weight: bold;
    }
    #dev-toolbar .dt-info {
    ...
    #dev-toolbar .dt-info span {
      color: #888;
      font-size: 11px;
    }
    #dev-toolbar .dt-btn {
      background: rgba(224, 111, 234, 0.15);
      border: 1px solid rgba(224, 111, 234, 0.3);
      color: #e06fea;
      padding: 2px 8px;
      border-radius: 3px;
      cursor: pointer;
    ...
    #dev-toolbar .dt-btn:hover {
      background: rgba(224, 111, 234, 0.3);
      color: #fff;
    }
    #dev-toolbar .dt-btn.dt-on {
      background: rgba(224, 111, 234, 0.35);
      color: #fff;
      border-color: #e06fea;
    }
    #dev-toolbar select.dt-select {
      background: rgba(20, 10, 30, 0.95);
      border: 1px solid rgba(224, 111, 234, 0.3);
      color: #e06fea;
      padding: 2px 4px;
    ...
    #dev-toolbar select.dt-select:focus {
      border-color: #e06fea;
    }
    #dev-toolbar select.dt-select option {
      background: #1a0a24;
      color: #e0e0e0;
    }
    #dev-toolbar select.dt-select optgroup {
      background: #1a0a24;
      color: #e06fea;
      font-style: normal;
    }
    #dev-toolbar .dt-device-label {
      color: #e06fea;
      font-size: 10px;
      font-weight: bold;
      white-space: nowrap;
  ...
  /* ── Device preview: resize the actual body ── */
  ...
  function updateDeviceLabel() {
    ...
    if (activeDevice && activeDevice.w > 0) {
      ...
      deviceLabel.textContent = `${dw}\u00D7${dh} ${orient}`;
  ...
  for (const p of pages) {
    ...
    if (currentFile === p.href) a.classList.add('dt-active');
    nav.appendChild(a);
  }
  bar.appendChild(nav);
  ...
  crtBtn.className = 'dt-btn';
  crtBtn.textContent = 'CRT Off';
  crtBtn.title = 'Toggle CRT effects';
  crtBtn.addEventListener('click', () => {
    const shell = document.querySelector('.crt-shell');
    if (!shell) return;
    const hidden = shell.style.display === 'none';
    shell.style.display = hidden ? '' : 'none';
    crtBtn.textContent = hidden ? 'CRT Off' : 'CRT On';
  });
  info.appendChild(crtBtn);

  /* Grid overlay */
  const gridBtn = document.createElement('button');
  gridBtn.className = 'dt-btn';
  gridBtn.textContent = 'Grid';
  gridBtn.title = 'Toggle layout grid overlay';
  let gridOn = false;
  let gridEl = null;
  gridBtn.addEventListener('click', () => {
    ...
    if (gridOn) {
      ...
      gridEl.id = 'dt-grid-overlay';
      gridEl.style.cssText = `
        position: fixed; inset: 0; bottom: ${TOOLBAR_H + 2}px;
        z-index: 2147483645; pointer-events: none;
        background-image:
          linear-gradient(rgba(224,111,234,0.08) 1px, transparent 1px),
          linear-gradient(90deg, rgba(224,111,234,0.08) 1px, transparent 1px);
        background-size: 20px 20px;
      `;
      document.documentElement.appendChild(gridEl);
      gridBtn.classList.add('dt-on');
    } else {
      gridEl?.remove();
      gridBtn.classList.remove('dt-on');
    }
  });
  info.appendChild(gridBtn);
